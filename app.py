...
if openai_key:
    ...
    if pdf_file:
        ...
        fn = str(pdf_file.name).split('.')[0]

        ### PARSING ###
        documents, texts = parse_pdf('temp.pdf')
        st.write(f"{len(texts)} text chunks created from {len(documents)} pages found in the pdf...   Storing embeddings into vectors' database...")
        ...
        ### EMBEDDING ###
        # Check if already in a persistent directory
